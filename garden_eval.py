...
parser.add_argument("--output_path", default="./eval_garden/eval")
...
parser.add_argument("--mv_scale_lr", action="store_true")
...
if args.mv is not None:
    ...
    if args.mv_equalize:
        train_filename = "train_mv_equalized.py"
        args.output_path += "_equalized"
        if args.mv_scale_lr:
            args.output_path += "_scaled_lr"
    else:
        train_filename = "train_mv_mvgs.py"
else:
...
if not args.skip_training:
    ...
    for scene in mipnerf360_outdoor_scenes:
        ...
        os.system(f"python {train_filename} -s " + source + " -i images_4 -m " + args.output_path + "/" + scene + common_args)
    for scene in mipnerf360_indoor_scenes:
        source = args.mipnerf360 + "/" + scene
        os.system(f"python {train_filename} -s " + source + " -i images_2 -m " + args.output_path + "/" + scene + common_args)
    m360_timing = (time.time() - start_time)/60.0

    start_time = time.time()
    for scene in tanks_and_temples_scenes:
        source = args.tanksandtemples + "/" + scene
        os.system(f"python {train_filename} -s " + source + " -m " + args.output_path + "/" + scene + common_args)
